...
from analytics.exceptions import NotFound
from analytics.db import Operation, PersonalBalance, BillingCycle
from analytics.dao.filters import make_string_filter


class DAOBilling:
    """
    DAO for interaction with 'operation', 'personal_balance' and 'analytics_cycle' tables
    """
    def __init__(self, engine):
        self.engine = engine

    async def create_new_analytics_cycle(self, start_date, end_date):
        async with self.engine.acquire() as conn:
            async with conn.begin():
                await self._create_new_analytics_cycle(conn, start_date, end_date)

    @staticmethod
    async def _create_new_analytics_cycle(conn, start_date, end_date):
        analytics_cycle = {
        ...
        await conn.execute(
            BillingCycle.insert().values(**analytics_cycle)
        )

    @staticmethod
    async def _get_current_analytics_cycle(conn) -> dict:
        row = await (
    ...
    async def get_current_analytics_cycle(self) -> dict:
        async with self.engine.acquire() as conn:
            return await self._get_current_analytics_cycle(conn)

    async def get_analytics_cycle(self, analytics_cycle_id: str) -> dict:
        async with self.engine.acquire() as conn:
            return await self._get_analytics_cycle(conn, analytics_cycle_id)

    @staticmethod
    async def _get_analytics_cycle(conn, analytics_cycle_id: str) -> dict:
        row = await (
            await conn.execute(BillingCycle.select().where(BillingCycle.c.id == analytics_cycle_id))
        ).first()
    ...
    async def close_analytics_cycle(self, analytics_cycle_id: str):
        async with self.engine.acquire() as conn:
            async with conn.begin():
                await self._close_analytics_cycle(conn, analytics_cycle_id)

    async def _close_analytics_cycle(self, conn, analytics_cycle_id: str):
        analytics_cycle = await self._get_analytics_cycle(conn, analytics_cycle_id)
        analytics_cycle[const.STATUS] = const.STATUS__BILLING_CYCLE__CLOSED
        await conn.execute(
            BillingCycle.update().values(**analytics_cycle).
            where(BillingCycle.c.id == analytics_cycle_id)
        )
    ...
    def _filtered_analytics_cycle_query(self, query, filter_: dict):
        for filtering_filed in filter_:
    ...
    @staticmethod
    def _ordered_analytics_cycle_query(query, filter_: List[dict]):
        if not filter_:
    ...
    async def get_analytics_cycle_count_by_filter(self, filter_: dict) -> int:
        async with self.engine.acquire() as conn:
            return await self._get_analytics_cycle_count_by_filter(conn, filter_)

    async def get_analytics_cycle_list_by_filter(self, filter_: dict, order: List[dict], limit: int, offset: int) -> List[dict]:
        async with self.engine.acquire() as conn:
            return await self._get_analytics_cycle_list_by_filter(conn, filter_, order, limit, offset)

    async def _get_analytics_cycle_count_by_filter(self, conn, filter_: dict) -> int:
        query = sqlalchemy.select([func.count(BillingCycle.c.id)])
        query = self._filtered_query(query, filter_)
        return await conn.scalar(query)

    async def _get_analytics_cycle_list_by_filter(self, conn, filter_: dict, order: List[dict], limit: int, offset: int) -> List[dict]:
